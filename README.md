# ATLAS Widget

Electron desktop app for IT support ticket submission, deployed to customer devices via NinjaOne.

## Current Status (v1.0.0)

- ✅ System tray icon with popup window
- ✅ Silent install via NinjaOne deployment scripts (Windows)
- ✅ Auto-start on login (Startup shortcut)
- ✅ Single-instance enforcement
- 🔜 Structured ticket form with categories
- 🔜 Device context collection (NinjaOne ID, TeamViewer ID)
- 🔜 WhatsApp QR code support
- 🔜 macOS deployment testing

## Project Structure

```
atlas-widget/
├── electron/                         # MAIN PROCESS (Node.js — runs in background)
│   ├── main.ts                       # App entry: tray icon, window, IPC handlers
│   ├── preload.ts                    # Security bridge between main & renderer
│   └── device-context.ts             # Reads device IDs from Windows registry
│
├── src/                              # RENDERER PROCESS (web page — what the user sees)
│   ├── index.html                    # Ticket submission form UI
│   ├── style.css                     # Dark theme styling
│   └── main.ts                       # Form logic, validation, API calls
│
├── scripts/                          # NINJAONE DEPLOYMENT SCRIPTS
│   ├── deploy-atlas-widget.ps1       # Windows install (PowerShell, run as SYSTEM)
│   ├── uninstall-atlas-widget.ps1    # Windows uninstall
│   ├── deploy-atlas-widget-mac.sh    # macOS install (Bash, run as root)
│   ├── uninstall-atlas-widget-mac.sh # macOS uninstall
│   ├── release.sh                    # Build + upload to S3 + tag (dev machine only)
│   └── rollback.sh                   # Roll back to previous version (dev machine only)
│
├── assets/                           # Icons (tray-icon.png, icon.ico, icon.icns)
├── dist/                             # Built renderer (generated by vite build)
├── dist-electron/                    # Built main process (generated by tsc)
├── release/                          # Final installers (generated by electron-builder)
│   ├── ATLAS Support Setup X.X.X.exe    # Windows NSIS installer
│   └── ATLAS Support-X.X.X-arm64.dmg   # macOS DMG
│
├── package.json                      # Dependencies + electron-builder config
├── tsconfig.json                     # TypeScript config for renderer (src/)
├── tsconfig.electron.json            # TypeScript config for main process (electron/)
└── vite.config.ts                    # Vite bundler config for the web UI
```

### How the Build Pipeline Works

```
1. tsc                          → Compiles src/main.ts → dist/
2. vite build                   → Bundles src/index.html + CSS + JS → dist/
3. tsc -p tsconfig.electron.json → Compiles electron/*.ts → dist-electron/
4. electron-builder             → Packages dist/ + dist-electron/ → release/*.exe / *.dmg
```

## Development

### Prerequisites

- Node.js 18+
- npm

### Setup

```bash
cd atlas-widget
npm install
```

### Run Locally (Dev Mode)

```bash
npm start
```

This compiles the Electron TypeScript and launches the app. You'll see a tray icon in
your menu bar (macOS) or system tray (Windows). Click it to see the popup.

### Available Scripts

| Command | What it does |
|---|---|
| `npm start` | Compile + launch Electron app locally |
| `npm run build:electron` | Compile only the main process (fast) |
| `npm run build:full` | Compile renderer + main process (no installer) |
| `npm run electron:build:win` | Full build + create Windows `.exe` installer |
| `npm run electron:build:mac` | Full build + create macOS `.dmg` installer |

## Environment Variables

Environment variables are set on the **end-user's machine** (not in a `.env` file) because
this is a desktop app, not a server. The deploy scripts set them during installation.

| Variable | Default | Set By | Description |
|---|---|---|---|
| `ATLAS_API_URL` | `http://localhost:3000/api` | Deploy script | Backend API URL for ticket submission |
| `ATLAS_INSTALLER_URL` | *(hardcoded in script)* | NinjaOne env var | Override installer download URL |

### How to use environment variables in the Electron code

In the main process (`electron/main.ts`), read them with:

```typescript
const apiUrl = process.env.ATLAS_API_URL || 'http://localhost:3000/api';
```

The deploy script sets machine-level env vars on Windows:

```powershell
[System.Environment]::SetEnvironmentVariable("ATLAS_API_URL", "https://your-backend-url/api", "Machine")
```

On macOS, the deploy script sets them via the LaunchAgent plist or `/etc/environment`.

## Deployment via NinjaOne

### One-Time Setup in NinjaOne

1. **Create Custom Fields** (Administration > Devices > Custom Fields):

   | Field Name | Type | Scope |
   |---|---|---|
   | `atlasWidgetInstalled` | Text | Device |
   | `atlasWidgetVersion` | Text | Device |
   | `atlasWidgetInstalledDate` | Text | Device |

2. **Upload Scripts** (Administration > Scripting > Add New Script):

   | Script | Language | Run As | File |
   |---|---|---|---|
   | ATLAS Widget - Deploy (Windows) | PowerShell | System | `scripts/deploy-atlas-widget.ps1` |
   | ATLAS Widget - Uninstall (Windows) | PowerShell | System | `scripts/uninstall-atlas-widget.ps1` |
   | ATLAS Widget - Deploy (macOS) | Shell (Bash) | Root | `scripts/deploy-atlas-widget-mac.sh` |
   | ATLAS Widget - Uninstall (macOS) | Shell (Bash) | Root | `scripts/uninstall-atlas-widget-mac.sh` |

### Deploy to a Single Device

1. NinjaOne > **Devices** > find the device
2. Click device > **Run Script** (or Actions > Run Script)
3. Select **"ATLAS Widget - Deploy (Windows)"**
4. Click **Run**
5. Check **Activities** tab for the result
6. Widget appears on next user login (Startup shortcut)

### Deploy to an Organization (Batch)

1. NinjaOne > **Devices** > filter by organization
2. Select all devices > **Actions > Run Script**
3. Select deploy script > **Run**

### Deploy via Policy (Auto-Deploy to New Devices)

1. NinjaOne > **Administration > Policies**
2. Edit the target policy > **Scripting** tab
3. Add the deploy script as a scheduled or condition-triggered script

### What the Deploy Script Does

1. Downloads the `.exe` from S3 (or overridden URL via `ATLAS_INSTALLER_URL` env var)
2. Runs silent NSIS installation to `C:\Program Files\ATLAS Support\`
3. Sets NinjaOne custom fields (`atlasWidgetInstalled = true`)
4. Launches the widget in the logged-in user's session
5. Creates a Startup shortcut for auto-start on login
6. Cleans up the downloaded installer

## Release Process

### Automated Release (Recommended)

One command that bumps version, builds installers, uploads to S3, updates deploy scripts,
and pushes to GitHub:

```bash
./scripts/release.sh patch   # bug fix:        1.0.0 → 1.0.1
./scripts/release.sh minor   # new feature:    1.0.0 → 1.1.0
./scripts/release.sh major   # breaking change: 1.0.0 → 2.0.0
```

**What `release.sh` does automatically:**

1. Bumps version in `package.json`
2. Builds Windows `.exe` and macOS `.dmg`
3. Uploads both to S3 (`s3://alphora-atlas-widget-releases/vX.X.X/`)
4. Uploads `latest.json` to S3 (deploy scripts read this to get the current version + URL)
5. Commits, tags `vX.X.X`, and pushes to GitHub
6. Prints the new S3 URLs

**The deploy scripts in NinjaOne never need updating** — they fetch `latest.json`
from S3 on every run to discover the current version and download URL.

**Prerequisites:**
- AWS CLI installed (`brew install awscli`)
- AWS credentials configured (`aws configure`)
- S3 bucket `alphora-atlas-widget-releases` exists with public read access

### How It Works (latest.json)

Every release uploads a `latest.json` file to the root of the S3 bucket:

```json
{
  "version": "1.2.0",
  "windows": "https://alphora-atlas-widget-releases.s3.eu-west-1.amazonaws.com/v1.2.0/ATLAS-Support-Setup-1.2.0.exe",
  "mac": "https://alphora-atlas-widget-releases.s3.eu-west-1.amazonaws.com/v1.2.0/ATLAS-Support-1.2.0-arm64.dmg",
  "released": "2026-02-12T14:30:00Z"
}
```

The deploy scripts fetch this file at runtime, extract the URL for their platform, and
download the installer. This means:

- ✅ Upload deploy scripts to NinjaOne **once**
- ✅ Run `./scripts/release.sh patch` to publish a new version
- ✅ Next NinjaOne deployment automatically uses the new version
- ❌ No need to edit NinjaOne scripts ever again

### Manual Release (Step by Step)

If you prefer to do it manually:

#### 1. Make changes and test locally

```bash
# Edit files in electron/ or src/
npm start  # test locally
```

#### 2. Bump version

```bash
npm version patch   # 1.0.0 → 1.0.1 (bug fix)
npm version minor   # 1.0.0 → 1.1.0 (new feature)
npm version major   # 1.0.0 → 2.0.0 (breaking change)
```

#### 3. Build installers

```bash
npm run electron:build:win   # Creates release/ATLAS Support Setup X.X.X.exe
npm run electron:build:mac   # Creates release/ATLAS Support-X.X.X-arm64.dmg
```

#### 4. Upload to S3

```bash
# Upload installers
aws s3 cp "release/ATLAS Support Setup X.X.X.exe" s3://alphora-atlas-widget-releases/vX.X.X/
aws s3 cp "release/ATLAS Support-X.X.X-arm64.dmg" s3://alphora-atlas-widget-releases/vX.X.X/

# Upload latest.json (so deploy scripts auto-discover the new version)
cat > /tmp/latest.json << EOF
{
  "version": "X.X.X",
  "windows": "https://alphora-atlas-widget-releases.s3.eu-west-1.amazonaws.com/vX.X.X/ATLAS-Support-Setup-X.X.X.exe",
  "mac": "https://alphora-atlas-widget-releases.s3.eu-west-1.amazonaws.com/vX.X.X/ATLAS-Support-X.X.X-arm64.dmg",
  "released": "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
}
EOF
aws s3 cp /tmp/latest.json s3://alphora-atlas-widget-releases/latest.json \
    --content-type "application/json" \
    --cache-control "no-cache, no-store, must-revalidate"
```

#### 5. Commit, tag, and push

```bash
git add -A
git commit -m "release: vX.X.X"
git push origin main

git tag vX.X.X
git push origin vX.X.X
```

## Rollback

If you release a broken version, you can roll back to **any** previous version.

### How It Works

Every release uploads two things to S3:

| File | Purpose |
|---|---|
| `latest.json` | Points to the newest version (what devices install by default) |
| `versions/vX.X.X.json` | Permanent record of every release (never overwritten) |

Even if you release multiple buggy versions in a row, every past version is
still available in `versions/`:

```
s3://alphora-atlas-widget-releases/
├── latest.json              ← v1.2.0 (current)
├── versions/
│   ├── v1.0.0.json          ← permanent, always available
│   ├── v1.1.0.json          ← even if buggy, still here
│   └── v1.2.0.json          ← current
├── v1.0.0/                  ← installers
├── v1.1.0/
└── v1.2.0/
```

### Option 1: Roll back from your dev machine (any version)

```bash
./scripts/rollback.sh              # Interactive: lists all versions, pick one
./scripts/rollback.sh v1.0.0       # Roll back to a specific version directly
./scripts/rollback.sh --list       # Just list available versions
```

This overwrites `latest.json` on S3 with the chosen version. All future
NinjaOne deployments will use that version.

### Option 2: Roll back a single device via NinjaOne

In NinjaOne, when running the deploy script on a device, set the script variable:

```
ATLAS_DEPLOY_MODE=v1.0.0          # Use a specific version
```

This only affects that specific run — it doesn't change `latest.json` globally.

### After Rolling Back

To re-deploy the rolled-back version on devices that already have the broken version,
just run the deploy script again via NinjaOne — it will now pick up the rolled-back version.

## Architecture Notes

- **Main process** (`electron/main.ts`): Creates the tray icon and BrowserWindow. Handles
  IPC messages from the renderer. Collects device context via `device-context.ts`.

- **Preload** (`electron/preload.ts`): Exposes a safe `window.atlasAPI` object to the
  renderer via `contextBridge`. The renderer cannot access Node.js directly (security).

- **Renderer** (`src/`): A vanilla HTML/CSS/JS web page. Communicates with the main process
  through `window.atlasAPI` (defined in preload). No frameworks (React, Vue, etc.) — kept
  simple intentionally.

- **Device context** (`electron/device-context.ts`): On Windows, reads NinjaOne device ID
  and TeamViewer ID from the Windows registry. On other platforms, returns what's available
  from Node.js OS APIs.

## Links

- **GitHub Releases:** https://github.com/alphora-holdings/atlas-widget/releases
- **Latest Windows Installer:** https://github.com/alphora-holdings/atlas-widget/releases/latest
